-- Prevent negative mileage in profiles table
ALTER TABLE public.profiles 
ADD CONSTRAINT mileage_non_negative CHECK (mileage >= 0);

-- Update purchase_item function to prevent negative mileage
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'success', false,
      'message', '존재하지 않거나 구매할 수 없는 아이템입니다.'
    );
  END IF;
  ...
  -- Deduct mileage (with non-negative check)
  UPDATE public.profiles
  SET mileage = GREATEST(0, mileage - v_item_price)
  WHERE id = v_user_id;
  ...
  VALUES (v_user_id, -v_item_price, '아이템 구매: ' || v_item_name);

  RETURN jsonb_build_object(
    'success', true,
    'message', v_item_name || ' 구매가 완료되었습니다!'
  );
END;
$$;

-- Drop and recreate admin_adjust_mileage with new return type
DROP FUNCTION IF EXISTS public.admin_adjust_mileage(uuid, integer, text);

CREATE FUNCTION public.admin_adjust_mileage(
  p_user_id uuid,
  p_amount integer,
  p_reason text
)
...
BEGIN
  ...
  IF v_current_mileage + p_amount < 0 THEN
    RETURN jsonb_build_object(
      'success', false, 
      'message', '마일리지는 음수가 될 수 없습니다. 현재 마일리지: ' || v_current_mileage || ', 변경 시도: ' || p_amount
    );
  END IF;

  -- Update mileage
  UPDATE public.profiles
  SET mileage = GREATEST(0, mileage + p_amount)
  WHERE id = p_user_id;
